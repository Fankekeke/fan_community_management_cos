<template>
  <a-modal v-model="show" title="修改周边商品" @cancel="onClose" :width="800">
...
export default {
  ...
  methods: {
    ...
    setFormValues ({...bulletin}) {
      ...
      let fields = ['name', 'category', 'price', 'stock', 'description', 'imageUrl', 'isOnSale']
      let obj = {}
      Object.keys(bulletin).forEach((key) => {
        if (key === 'isOnSale') {
          obj[key] = bulletin[key].toString()
        }
        if (key === 'imageUrl') {
          this.fileList = []
          this.imagesInit(bulletin['imageUrl'])
        }
